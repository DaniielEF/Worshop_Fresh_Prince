document.addEventListener('DOMContentLoaded', () => {
    ...
    loadProducts();
    // Session Storage para el inicio obligatorio del usuario en la página principal

    JSON.parse(sessionStorage.getItem('Authentication')) ? sessionStorage.setItem("Authentication", 'true') : sessionStorage.setItem("Authentication", 'false');
    const Authentic = sessionStorage.getItem('Authentication');
    ...
    if (Authentic !== 'true') {
        setTimeout(() => {
            console.log('ahora se envia el login')
            const modalLogin = new bootstrap.Modal(document.getElementById('alertModal'))
            modalLogin.show();
        }, 2000);
    } else {
        ...
        welcomeMessage.innerHTML = `
            <h1><b>Welcome, ${user.name}.<br>
            Find The Perfect Place to<br>
                Live With your Family</b></h1>
        `
        if (user.email === 'Administrator@gmail.com') {
            document.getElementById('addProperty').style.visibility = 'visible'
